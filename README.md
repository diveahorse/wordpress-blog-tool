...
ブラウザで `http://localhost:5000` にアクセスして、直感的なGUIでブログ記事を生成できます。

### Webアプリケーションの機能
- **直感的なGUI**: フォームベースの簡単操作
- **プロンプトテンプレート選択**: ニュース用、ランキング用、カスタム用から選択
- **リアルタイム進捗表示**: 記事生成の進行状況を視覚的に確認
- **接続テスト**: Perplexity APIとWordPressの接続状態を確認
- **生成履歴管理**: 過去に生成した記事の一覧表示・削除
- **記事プレビュー**: 生成された記事の内容を確認
- **レスポンシブデザイン**: スマートフォンやタブレットでも使用可能

### Webアプリケーションの画面構成
1. **ホーム画面**: 記事生成フォームと接続テスト
2. **履歴画面**: 生成された記事の一覧と管理
3. **記事表示画面**: 生成された記事の詳細表示
...
3. **記事生成手順**
   - 接続テストを実行してAPIの動作を確認
   - 記事のテーマを入力
   - プロンプトテンプレートを選択（標準/アニメランキング/カスタム）
   - 投稿ステータスを選択（下書き/公開）
   - 最大トークン数を選択（記事の長さ）
   - 「記事を生成して投稿」ボタンをクリック

4. **生成履歴の確認**
   - ナビゲーションの「履歴」から過去の記事を確認
   - 記事の表示・削除が可能
